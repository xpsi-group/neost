...
from matplotlib.colors import ListedColormap                     ##### CAUTION: alterations for including new sampling parameter: ceft_in (polytrope index) 
...
def compute_table_data(root_name, EOS, variable_params, static_params):
    ...
    try:
        ...
        for i in range(0, len(ewposterior), 1):
            ...
            eps = numpy.logspace(14.2, numpy.log10(EOS.max_edsc), 40)    ###### need to reduce this number for NLO?
...
def compute_prior_auxiliary_data(root_name, EOS, variable_params, static_params):
    ...
    energydensities = numpy.logspace(14.2, 16, 50)                       ###### also need to reduce this to around 8 for NLO?
...
def compute_cs(root_name, EOS, variable_params, static_params): 
    ewposterior = numpy.loadtxt(root_name) # + 'post_equal_weights.dat')
    ...
    energy_d = numpy.zeros((len(masses), len(ewposterior)))
    #mass_d = numpy.zeros((len(masses), len(ewposterior)))
    cs = numpy.zeros((len(masses), len(ewposterior)))
    ...
    for i in range(0, len(ewposterior), 1):
        ...
        pressures_rho[:,i][energydensities<max_rhoc] = rhopres(energydensities[energydensities<max_rhoc])
        pressures[:,i][energydensities<EOS.max_edsc] = EOS.eos(energydensities[energydensities<EOS.max_edsc])
        energy_d[:,i][energydensities<EOS.max_edsc] = energydensities[energydensities<EOS.max_edsc]
        #mass_d[:,i][EOS.massdensities<max_rhoc] = EOS.massdensities[EOS.massdensities<max_rhoc]
        spl = EOS.eos.derivative(1)
        cs[:,i][energydensities<EOS.max_edsc] = spl(energydensities[energydensities<EOS.max_edsc])

    numpy.save(root_name + 'pressures', pressures)
    numpy.save(root_name + 'pressures_rho', pressures_rho)
    numpy.save(root_name + 'energydensity', energy_d)
    numpy.save(root_name + 'cs', cs)
    #numpy.save(root_name + 'rho', mass_d)
...
def compute_auxiliary_data(root_name, EOS, variable_params, static_params, chirp_masses): 
    ...
    masses = numpy.linspace(.2, 2.9, 50)
    energydensities = numpy.logspace(14.2, 16, 50)                  ################### also reduce here for NLO
    scattered = []
    ...
    for i in range(0, len(ewposterior), 1):
        ...
        rhocs = numpy.logspace(14.2, numpy.log10(EOS.max_edsc), 30)           ############### reduce here for NLO
        ...
        rhoc = numpy.random.rand() *(numpy.log10(EOS.max_edsc) - 14.2) + 14.2           ############# reduce here for NLO
        star = Star(10**rhoc)
        star.solve_structure(EOS.energydensities, EOS.pressures)
        MR_prpr_pp[i] = star.Mrot, star.Req
