...
N2LO_beta = np.loadtxt(f'{file_path}/N2LO_beta_X2D.txt')                        
N2LO_pnm = np.loadtxt(f'{file_path}/N2LO_PNM_X2D.txt')
...
corr_N3LO = correlation(N3LO_beta, N3LO_pnm, densities, quantiles, number_density)   
corr_N2LO = correlation(N2LO_beta, N2LO_pnm, densities, quantiles, number_density)

class class_L_inf:

    def __init__(self, order_nice):
        print('Starting L inference')

    def corr(self, order_nice):
        if order_nice == 'G-N2LO':
            self.corr = corr_N2LO
        elif order_nice == 'G-N3LO':
            self.corr = corr_N3LO
        return self.corr
