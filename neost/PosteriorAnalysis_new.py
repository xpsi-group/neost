...
def compute_auxiliary_data(path, EOS, variable_params, static_params, chirp_masses, dm=False, sampler='multinest', identifier='', freq=None):
    """
    ...
    samples = comm.scatter(samples, root=0)
    results = _compute_auxiliary_data_thread(samples, EOS, variable_params, static_params, chirp_masses, dm, eos_is_fixed, mpi_rank, freq)
...
def _compute_auxiliary_data_thread(samples, EOS, variable_params, static_params, chirp_masses, dm, eos_is_fixed, thread_number, freq):
    '''
        ...
        if not dm:
            ...
            for j, e in enumerate(rhocs):
                ...
                if freq is None:
                    ...
                    star.solve_structure(EOS.energydensities, EOS.pressures, rot_cor='Uni_rel', freq=np.random.choice(freq))
                M[j] = star.Mrot
                R[j] = star.Req

            masses = np.linspace(min(M), max(M), num_grid_points)
            MR= interp1d(M, R, kind='linear')
            #M, indices = np.unique(M, return_index=True)
            #MR = UnivariateSpline(M, R[indices], k=1, s=0, ext=1)
            
            #rhocM = UnivariateSpline(M, rhocs[indices], k=1, s=0, ext=1)
            rhocM = interp1d(M, rhocs, kind='linear')
            ...
            for j, e in enumerate(rhocs):
                ...
                if freq is None:
                    ...
                    star.solve_structure(EOS.energydensities, EOS.pressures,EOS.energydensities_dm, EOS.pressures_dm, EOS.dm_halo, rot_cor='Uni_rel', freq=np.random.choice(freq))
                M[j] = star.Mrot
                R[j] = star.Req
...
def mass_radius_prior_predictive_plot(path, variable_params, identifier='', label_name='updated prior'):
    ...
    if eos_is_fixed:
        ...
        inbins = np.histogramdd(x, bins=50, density=True)
        levels = [0.05, 0.32, 1]
        ...
        ax.set_ylim(1., 3.)
        ax.set_xlim(9.05, 15)
        fig.suptitle('Mass-Radius Prior Predictive', fontsize=20)
        ax.set_title('500 Hz, Analysed using 0 Hz', fontsize=14)
        plt.tight_layout()
        plt.show()
        fig.savefig(f'{path}/{identifier}_new_MRpriorpredictive.png')
