...
import matplotlib.pyplot as plt      ## for debbuging
...
class PolytropicEoS(BaseEoS):
    ...
    def get_eos(self):
        ...
        if self._eds_crust[-1] == self._eds_core[0]:
            ...
            #print(list(self.eos_params.values()))
            
        if self._pres_crust[-1] == self._pres_core[0]/dyncm2_to_MeVfm3:
            print('same pressure at transition')
            #print(list(self.eos_params.values()))
        
        if all(x<y for x, y in zip(self._eds_crust, self._eds_crust[1:]))==False:
            print('energy density of the crust not monotonically increasing')
            #print(list(self.eos_params.values()))
        ...
        if all(x<y for x, y in zip(self._pres_crust, self._pres_crust[1:]))==False:
            print('pressure of the crust not monotonically increasing')
            #print(list(self.eos_params.values()))
            print(self._pres_crust)
                
        if all(x<y for x, y in zip(self._pres_core/dyncm2_to_MeVfm3, self._pres_core[1:]/dyncm2_to_MeVfm3))==False:
            print('pressure of the core not monotonically increasing')
            #print(list(self.eos_params.values()))
            
        fig, ax = plt.subplots(1,1, figsize=(8,6))
        ax.plot(self.energydensities, self.pressures, color='black', marker='o', linestyle='-')
        ax.plot(self.ceft_energy[self.index_start_cEFT:]/gcm3_to_MeVfm3, self.ceft_pressure_werror[self.index_start_cEFT:]/dyncm2_to_MeVfm3, color='red', linestyle='--')
        ax.axvline(self.eds_t, ymin=0, ymax=1, color='black', linestyle='--', label='end chiral')
        ax.axvline(self.epsBPS[-1], ymin=0, ymax=1, color='red', linestyle='--', label ='end BPS')
        ax.set_xscale('log')
        ax.set_yscale('log')
        
        ax.set_xlim(self.epsBPS[-7], 1e+15) #a bit random choice
        ...
        plt.tight_layout()
        fig.savefig(f'./fig/ceft_'+str(1.90734863e-06)+'.png')   ## for multiple eos plotting
        plt.show()
