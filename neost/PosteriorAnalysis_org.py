...
def compute_table_data(root_name, EOS, variable_params, static_params):
    ...
    try:
        ...
        Data_array = np.zeros((len(ewposterior),4)) #contains Mtov, Rtov, eps_cent TOV, rho_cent TOV, P_cent TOV,R 1.4, eps_cent 1.4, rho_cent 1.4,
        ...
        for i in tqdm(range(0, len(ewposterior), 1)):
            ...
            row = [EOS.max_M, EOS.Radius_max_M,R_14,R_2]
...
def compute_table_data_de(root_name, EOS, variable_params, static_params):
    ewposterior = np.loadtxt(root_name + 'post_equal_weights.dat')
    print("Total number of samples is %d" %(len(ewposterior)))
    try:
        Data_array = np.loadtxt(root_name + 'table_data.txt')
        ...
        Data_array = np.zeros((len(ewposterior),4)) 

        for i in tqdm(range(0, len(ewposterior), 1)):
            pr = ewposterior[i][0:len(variable_params)]
            par = {e:pr[j] for j, e in enumerate(list(variable_params.keys()))}
            par.update(static_params)
            EOS.update(par, max_edsc_de=True)

            #print(EOS.max_edsc_de)


            eps = np.linspace(EOS.rho_plus/rho_ns +0.1, EOS.max_edsc_de, 50)*rho_ns
            M = np.zeros(len(eps))
            R = np.zeros(len(eps))
            for j, e in enumerate(eps):
            ...
            max_M = max(M)
            index_max_M = np.argmax(M)
            Radius_max_M = R[index_max_M]

            row = [max_M, Radius_max_M,R_14, R_2]
            #print(row)
            for k in range(len(row)):
                # Some of the values in row may be arrays of shape (1,),
                # which causes the line "Data_array[i] = row" to fail for np > 1.23.5.
...
def compute_auxiliary_data_de2(root_name, EOS, variable_params, static_params, prior = False):
    ...
    energydensities = np.linspace(eps_min, eps_max, 50)*rho_ns

    if flag == True:
        pressures_baryon = np.zeros((len(masses), len(ewprior)))
        pressures_total = np.zeros((len(masses), len(ewprior)))
        ...
        minpres_baryon = np.zeros((3, len(energydensities)))
        maxpres_baryon = np.zeros((3, len(energydensities)))
        MR_prpr_pp = np.zeros((len(ewprior), 2))

    for i in tqdm(range(0, len(ewprior), 1)):

        pr = ewprior[i][0:len(variable_params)]
        par = {e:pr[j] for j, e in enumerate(list(variable_params.keys()))}
        par.update(static_params)
        EOS.update(par, max_edsc_de=True, max_edsc = True)


        energydensities = np.logspace(14.2,16,50)
        pressures_baryon[:,i][energydensities<EOS.max_edsc] = EOS.eos(energydensities[energydensities<EOS.max_edsc])
        #This loop sequence is for the neutron stars with dark energy cores

        eps_min = 10**(14.2)/rho_ns
        eps_max = 10**(16)/rho_ns
        energydensities = np.linspace(eps_min, eps_max, 50)*rho_ns
        for j in range(len(energydensities)):
            if EOS.rho_plus < energydensities[j]:
                if energydensities[j]/rho_ns < EOS.max_edsc_de:
                    pressures_total[:,i][j] = EOS.eos_de(energydensities[j])
            else:
                if energydensities[j]/rho_ns < EOS.max_edsc_de:
                    pressures_total[:,i][j] = EOS.eos(energydensities[j])
    np.save(root_name + 'pressures_total', pressures_total)
    np.save(root_name + 'pressures_baryon', pressures_baryon)

    minpres, maxpres = calc_bands(energydensities, pressures_total)
    np.save(root_name + 'minpres_total', minpres)
    np.save(root_name + 'maxpres_total', maxpres)

    minpres_baryon, maxpres_baryon = calc_bands(energydensities, pressures_baryon)
    np.save(root_name + 'minpres_baryon', minpres_baryon)
    np.save(root_name + 'maxpres_baryon', maxpres_baryon)

def compute_auxiliary_data_de(root_name, EOS, variable_params, static_params, prior = False):
    ewprior = np.loadtxt(root_name + 'post_equal_weights.dat')
    ...
    for i in tqdm(range(0, len(ewprior), 1)):
        ...
        for j in range(len(energydensities)):
            if EOS.rho_plus < energydensities[j]:
                if energydensities[j]/rho_ns < EOS.max_edsc_de:
                    pressures_total[:,i][j] = EOS.eos_de(energydensities[j])
                 #dark energy contribution for central energy densities in which rho_plus is larger than
                    #print(np.log10(EOS.rho_plus), np.log10(energydensities[j]), np.log10(EOS.max_edsc_de*rho_ns), EOS.eos_de(energydensities[j]))
            else:
                if energydensities[j]/rho_ns < EOS.max_edsc_de:
                    pressures_total[:,i][j] = EOS.eos(energydensities[j])
                #print(EOS.eos(energydensities[j] < EOS.max_edsc_de*rho_ns))


        energydensities = np.logspace(14.2,16,50)
        pressures_baryon[:,i][energydensities<EOS.max_edsc] = EOS.eos(energydensities[energydensities<EOS.max_edsc]) #computing this for the entire baryonic EoS only without a dark energy core

        if prior is False:
            rhoc = np.random.rand() *(np.log10(EOS.max_edsc_de*rho_ns) - np.log10(EOS.rho_plus)) + np.log10(EOS.rho_plus)
            rhoc = 10**rhoc
            ...
            #print(par, np.log10(rhoc), EOS.eos_de(rhoc))
...
def compute_table_data_ADM(root_name, EOS, variable_params, static_params):
    ewposterior = np.loadtxt(root_name + 'post_equal_weights.dat')
    print("Total number of samples is %d" %(len(ewposterior)))
    try:
        Data_array = np.loadtxt(root_name + 'table_data.txt')
        ...
        for i in tqdm(range(0, len(ewposterior), 1)):
            ...
            for j, e in enumerate(eps):
                ...
                if EOS.reach_fraction == False or star.Req == 0: #in the event of a halo
                    #print(star.Mdm/star.Mrot*100, EOS.adm_fraction)
                    M[j] = 0
                    R[j] = 0 #just a simple value to easily detect
...
def compute_auxiliary_data_ADM(root_name, EOS, variable_params, static_params, prior = False):
    ewprior = np.loadtxt(root_name + 'post_equal_weights.dat')
    print("Total number of samples is %d" %(len(ewprior)))
    ...
    energydensities_dm = np.logspace(10, 18, 250)
    ...
    for i in tqdm(range(0, len(ewprior), 1)):
        ...
        #energydensities_new = np.logspace(14.2,17,250) maybe??
        #presses = EoS.eos(energydensities_b) + eos_dm(energydensities_B)
        #admixed_eos = UnivariateSpline(energydensities_b, presses, k=1, s=0, ext = 1)
        #then you can compute the following
        #pressures[:,i][energydensities_{new or b}< max_edsc_admixed] = admixed_eos(energydensities_{new or b}[energydensities_{new or b}<max_edsc_admixed])

        if len(dm_pres_eps) != 0:
            # print(dm_pres_eps)
            # print(energydensities_dm[energydensities_dm<max_edsc_dm])
            # print(max_edsc_dm/1e13)
            # print(EOS.eos_params, EOS.ceft_param)
            ...
            #Only want to see impact of ADM where we have across the baryonic central energy densities 
            # 
        ...
        if prior is False:
            ...
            star.solve_structure(EOS.energydensities, EOS.pressures,EOS.energydensities_dm, EOS.pressures_dm,EOS.dm_halo)
            MR_prpr_pp[i] = star.Mrot, star.Req
            #print(star.Mrot,star.Req, star.Mdm/star.Mrot*100, EOS.adm_fraction, EOS.reach_fraction)
    ...
    # save everything
    np.save(root_name + 'pressures_again', pressures)
    np.savetxt(root_name + 'MR_prpr_again.txt', MR_prpr_pp)
    ...
    np.save(root_name + 'minpres_again', minpres)
    np.save(root_name + 'maxpres_again', maxpres)
...
#Keep in the even that it might be useful, but really rasises the maximum pressure band
# def compute_auxiliary_data_ADM_pressures(root_name, EOS, variable_params, static_params, prior = False):
    
#         max_edsc_admixed = EOS.max_edsc + max_edsc_dm

#         dm_pres_eps = eos_dm(energydensities_dm[energydensities_dm<max_edsc_dm])

#         energydensities_new = np.logspace(14.2,17,250)
#         presses = EOS.eos(energydensities_new) + eos_dm(energydensities_new)
#         admixed_eos = UnivariateSpline(energydensities_new, presses, k=1, s=0, ext = 1)
#         #then you can compute the following
#         pressures[:,i][energydensities_new< max_edsc_admixed] = admixed_eos(energydensities_new[energydensities_new<max_edsc_admixed])